...
class GiasService {
    ...
    async fetchContractData(id) {
        try {
            const response = await this.instance.get(`https://www.gias.by/purchase/api/v1/purchase/${id}`);
    ...
    async fetchPurchasesData(textSearch) {
        try {
            ...
            const contractPromises = contractsId.map(id => this.fetchContractData(id));
...
module.exports = new GiasService();